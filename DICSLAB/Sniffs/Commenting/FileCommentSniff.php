...
class DICSLAB_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected function processPackage($errorPos)
    {
        ...
        if ($package !== null) {
            ...
            if (empty($content) === true) {
                ...
                $this->currentFile->addError($error, $errorPos, 'DicslabPackage', $data);
            }
        }
    ...
    protected function processAuthors($errorPos)
    {
        ...
        if (empty($authors) === false) {
            ...
            } else if ($content !== 'DICSLAB LLC <products@dicslab.com>') {
                $error = 'Expected "DICSLAB LLC <products@dicslab.com>" for author tag';
    ...
    /**
     * Copyright tag must be in the form '2013-YYYY xxxx DICSLAB LLC (EDRPOU 38879176)'.
     *
     * @param int $errorPos The line number where the error occurs.
     *
     * @return void
    ...
    protected function processCopyrights($errorPos)
    {
        ...
        if ($copyright !== null) {
            ...
            } else if (preg_match('/^([0-9]{4})(-[0-9]{4})? (DICSLAB LLC \(EDRPOU 38879176\))$/', $content) === 0) {
                $error = 'Expected "xxxx-xxxx DICSLAB LLC (EDRPOU 38879176)" for copyright declaration';
